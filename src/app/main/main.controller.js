(function() {
  ...
  angular
    .module('hosen')
    .controller('MainController', ['$scope', '$location', '$anchorScroll', 'AuthenticationService', '$rootScope' , '$http', '$window', 'config',
  function ($scope, $location, $anchorScroll, AuthenticationService, $rootScope, $http, $window, config) {
    ...
    vm.aboutDesc = '為2016年創立的一家台灣網路郵購服飾品牌，其以UNIQLO的廉價時尚為構想，同時設計符合台灣副熱帶氣候的衣服，並加入多樣化圖案元素，而且由網路購物發跡，非實體店面。2016年營業額為臺灣網路服飾品牌第一，名稱CxN Boutique由來為創辦人Celia與Nina的縮寫';
    vm.newsDesc = '因教育訓練 1/19​(三)​~1/21​(五) 將暫停退貨處理及退貨退款作業, 8/24(一)恢復正常作業，期間不影響線上退貨申請作業​。造成您的不便，盡請見諒。<br>C&N 祝您平安順心';
  ...
  .directive('navbar', [ '$location' , '$window' , 'AuthenticationService', '$http' , 'config',  function($location, $window, AuthenticationService, $http, config) {
    return {
      ...
      link: function($scope, $element,$attrs){
            $scope.signInOut = function() {
              if(!$scope.navbar.IsLogin){
                ...
                AuthenticationService.ClearCredentials();
                $http({
                      url: config.myDomianName + '/api/logout',
                      method: "POST",
                      withCredentials: true,
                      headers: {
                                  'Content-Type': 'application/json; charset=utf-8'
